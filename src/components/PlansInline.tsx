...
import {
  activateLtcSubscription,
  LTC_PAY_ADDRESS,
  LTC_TIERS,
} from "@/lib/ltc-pay.functions";
...
const FEATURES_LEFT = BaseProFeatures.slice(
  0,
  Math.ceil(BaseProFeatures.length / 2),
);

const FEATURES_RIGHT = BaseProFeatures.slice(
  Math.ceil(BaseProFeatures.length / 2),
);
...
export function PlansInline({ compact = false }: { compact?: boolean }) {
  ...
  async function confirm() {
    ...
    try {
      ...
      if (r.ok) {
        toast.success("Heightened Security activated! Reloading...");
              ...
              <div className="rounded-xl border border-border bg-background/60 p-3 text-xs space-y-3">
                ...
                <div className="pt-2 border-t border-border/60 space-y-2">
                  ...
                  <p className="flex items-start gap-1.5 text-[10px] text-muted-foreground">
                    ...
                    <span>
                      Activation is verified on the Litecoin blockchain. Once
                      your txid confirms, Pro unlocks instantly. Rate $
                      {ltcUsd.toFixed(2)}/LTC.
                    </span>
                  </p>
                </div>
              </div>
    ...
    <div className="rounded-2xl border border-primary/30 bg-primary/5 p-4 space-y-4">
      <div className="text-center">
        ...
        <p className="text-sm text-muted-foreground">
          Heightened Security for Litecoin — advanced wallet protection,
          security intelligence, quantum-readiness planning, continuous
          monitoring, and unlimited AI assistance
        </p>
            ...
            <div className="rounded-2xl border border-primary/40 bg-background/60 p-4 space-y-4">
              ...
              <div className="pt-3 border-t border-border/60 space-y-3">
                ...
                <p className="flex items-start gap-2 text-xs text-muted-foreground/70">
                  ...
                  <span>
                    Activation is verified on the Litecoin blockchain. Once
                    your txid confirms, Pro unlocks instantly. Current rate:{" "}
